...
digraph workflow {
    ...
    F ->|"no"| H["Write SKILL.md"];
    G -> I["Parameterize all hardcoded values"];
    I -> H;
    H -> J{"Has subagents?"};
    J ->|"yes"| K["Write subagent prompt template"];
    J ->|"no"| L["Verify & deploy"];
    K -> L;
}
```
...
2. **Gather requirements** — Collect skill purpose, target use cases, and triggering conditions
3. **Analyze source** — If a specific prompt was provided, identify all hardcoded specifics
...
7. **Write skill** — Generate the complete SKILL.md
8. **Define subagents** — If the skill spawns subagents, write their prompt templates
...
- Use `CONSTANT_CASE` for variable names
...
## Step 8: Define Subagent Prompts

If the skill dispatches subagents, include a parameterized prompt template.

### Template Structure

Include a `## Subagent Prompt Template` section with instructions to replace `[variable]` with configuration values. The template should contain:

- Parameterized prompt text with backtick-wrapped variables
...
Include all configuration variables that a subagent needs. Use backtick-wrapped variable names (`VAR_NAME`) throughout the template.
...
When you generate a skill, here are common variable patterns to consider:

| Pattern | Variable Name | Example Use |
|---------|---------------|-------------|
| Input file | `INPUT_FILE` or `SOURCE_FILE` | Read existing content |
| Output file | `OUTPUT_FILE` or `TARGET_FILE` | Write generated content |
| Directory | `OUTPUT_DIR` or `TARGET_DIR` | Where results live |
| Technology | `TECH_STACK` | Language, framework, domain context |
| Project | `PROJECT_NAME` | Project identifier |
| Style | `STYLE` or `FORMAT` | Output style preference |
| Scope | `SCOPE` or `DEPTH` | Analysis breadth |
...
<!-- ✅ GOOD -->
Read from `PLAN_FILE` (default: `docs/plan.md`)
```
...
```markdown
<!-- ❌ BAD: hardcoded tech stack -->
The project uses React with TypeScript

<!-- ✅ GOOD -->
The project uses `TECH_STACK` (default: `React with TypeScript` / example needed)
```
...
Base directory for this skill: /mnt/c/Users/Danni/Documents/Git/opencode-tooling/skill/creating-skills
