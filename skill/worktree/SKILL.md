...
After `create_worktree` succeeds, return its output exactly and verbatim in a fenced code block. The tool output is the canonical absolute path on the OpenCode server. Do not reconstruct, shorten, rewrite, or otherwise replace it.
